        ...
        <div className=' pt-4 border border-t-0'>
          {
            dashData.latestAppointments.map((items,index)=>(
              <div className=' flex items-center gap-3 px-6 hover:bg-gray-100 py-3' key={index}>
                ...
                :
                items.isCompleted ? <p className=' text-green-500 font-medium text-xs'>Completed</p>
                : 
                <div className=' flex '>
                  <img className='w-10 cursor-pointer' src={assets.cancel_icon} onClick={()=>cancelAppointment(items._id)} alt="" />
                  <img className='w-10 cursor-pointer' src={assets.tick_icon} onClick={()=>completeAppointment(items._id)} alt="" />
                </div>
              }
              </div>
            ))
